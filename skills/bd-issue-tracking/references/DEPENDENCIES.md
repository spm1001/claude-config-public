...
**CRITICAL**: The actual pattern is **INVERTED** from what intuition suggests!

**MCP pattern**: Use `mcp__plugin_beads_beads__dep` with:
```json
{
...
**CLI pattern**: `bd dep add DEPENDENT PREREQUISITE`
```bash
...
**Direction matters**: `from_issue` depends on `to_issue`, which means **`to_issue` must complete before `from_issue` can start**.

**Mental model trap**: The parameter names are counterintuitive!
- `from_issue: A, to_issue: B` does NOT mean "A blocks B"
- It means "A depends on B" (B blocks A)
- `from_issue` = what waits (the dependent)
- `to_issue` = what must finish first (the prerequisite)

**Verification**: After creating, check with `mcp__plugin_beads_beads__show`:

Use `mcp__plugin_beads_beads__show` with `issue_id: "blocked-issue"`

Should display: "Dependencies (blocks this issue): prerequisite-issue"

(CLI: `bd show blocked-issue`)

If prerequisite shows under "Dependents" instead, you got the direction backwards.

**Real-world examples with CORRECT inverted syntax:**

```bash
# Example 1: Database before API
bd dep add api-endpoint db-schema
bd show api-endpoint  # Should show db-schema under "Dependencies"
# ✓ Correct: api-endpoint depends on db-schema

# Example 2: Phase ordering
bd dep add phase2 phase1
bd show phase2  # Should show phase1 under "Dependencies"
# ✓ Correct: phase2 depends on phase1

# Example 3: Setup before feature
bd dep add feature-profiles setup-auth
bd show feature-profiles  # Should show setup-auth under "Dependencies"
# ✓ Correct: feature depends on setup

# WRONG (what intuition suggests):
bd dep add phase1 phase2  # ✗ This makes phase1 depend on phase2 (backwards!)
```

**Memory aid**: "The thing that waits goes first in the command"
